...
import { decryptBase64, decryptWithAes, encryptBase64, encryptWithAes, generateAesKey } from '#/utils/old/encryption/crypto';
import * as encryptUtil from '#/utils/old/encryption/jsencrypt';
...
function createRequestClient(baseURL: string) {
  ...
  client.addResponseInterceptor(errorMessageResponseInterceptor((msg: string) => ElMessage.error(msg)));
  ...
  client.addResponseInterceptor<HttpResponse>({
    fulfilled: async (response) => {
      ...
      if (encryptKey) {
        ...
        const base64Str = encryptUtil.decrypt(encryptKey as string);
      ...
      if (hasSuccess) {
        ...
        if (response.config.successMessageMode === 'modal') {
          ...
          // });
          ElMessageBox.alert(successMsg, $t('http.successTip'), {});
      ...
      if (response.config.errorMessageMode === 'modal') {
        ElMessageBox.alert(timeoutMsg, $t('http.errorTip'), {});
